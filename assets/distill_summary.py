...
def distill_summary(combined_annotations_path, target_id_counts_df, output_path):
    ...
    for distill_sheet in distill_sheets:
        ...
        for common_gene_id_column in potential_gene_id_columns:
            # Ensure no duplicate columns (other than 'gene_id') will be created during merge
            distill_df_unique = distill_df.drop(columns=combined_annotations_df.columns.difference(['gene_id']), errors='ignore')
            merged_df = pd.merge(
