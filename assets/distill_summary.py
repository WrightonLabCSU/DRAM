...
def partial_match(gene_id, combined_column):
    if isinstance(gene_id, pd.Series):
        return combined_column.astype(str).apply(lambda x: any(g_id in x.split(';') for g_id in gene_id))
    else:
        return combined_column.astype(str).apply(lambda x: gene_id in x.split(';'))
