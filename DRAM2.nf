...
if (params.distill_custom != "") {
    ...
    ch_distill_custom_temp = customChannels.size() > 0 ? Channel.fromList(customChannels) : Channel.from(default_channel).ifEmpty(default_channel)
    ch_distill_custom_temp.collect().each { println it }
