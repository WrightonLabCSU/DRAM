...
from definitions import DRAM_DIR
...
@click.option('--output_dir', type=click.Path(), default='./DRAM_output.{datetime}',
...
@click.pass_context
def cli(ctx, output_dir, input_fasta,):
    """Main CLI tool."""
    # click.echo(ctx.invoked_subcommand)
    if ctx.invoked_subcommand is None:
        click.echo(ctx.get_help())
        return
    if output_dir == './DRAM_output.{datetime}':
        output_dir = str(Path.cwd() / f'DRAM_output.{dt.datetime.now().strftime("%Y%m%dT%H%M%S")}')
...
def call(rename, prodigal_mode, prodigal_tras_table, min_contig_len):
...
@cli.command()
@click.option('--kegg_pep_loc', type=click.Path(),
...
@click.option('--kegg_pep_root_dir', type=click.Path(),
