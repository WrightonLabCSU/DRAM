process RRNA_COLLECT {
    ...
    # Initialize list for storing combined data.
    combined_data = []
    ...
    for file in tsv_files:
        with open(file, 'r') as f:
            ...
            if contents == "NULL":
                print(f"File {file} contains 'NULL'. Skipping...")
                continue
            ...
            for index, row in df.iterrows():
                ...
                print(f"Extracted gene ID: {gene_id} from note: {row['note']}")
                gene_type_counts[gene_id][row['sample']] += 1
            
            combined_data.append(df)

    if all_files_null:
        print("All input files contain 'NULL'. Writing 'NULL' to output files.")
