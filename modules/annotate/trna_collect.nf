process TRNA_COLLECT {
    ...
    collected_data['gene_description'] = collected_data['gene_id'].str.replace(r'\([^)]*\)', '', regex=True).astype(str) + " tRNA with " + collected_data['codon'] + " Codon"
    collected_data['module'] = collected_data['gene_id'].str.replace(r'\([^)]*\)', '', regex=True).astype(str) + " tRNA"
