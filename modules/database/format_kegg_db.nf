process FORMAT_KEGG_DB {
    ...
    script:
    """
    if [ ${params.skip_gene_ko_link} ]; then
        echo "No Gene KO Link file provided. Running KEGG DB formatting without"
        python ${ch_format_kegg_db_script} --kegg_loc ${ch_kegg_pep} --download_date ${kegg_download_date} --threads ${params.threads} --output_dir kegg --skip_gene_ko_link
    else
