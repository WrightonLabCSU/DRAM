...
`auxiliary_score` is the v1 flank-confidence score (1 = hallmark VirSorter genes on both sides, 5 = no viral context, B-flag genes are bumped to 4). Without external category input every gene scores 5 — the v1 fallback. Pipe in a geNomad `*_genes.tsv` (one per sample, glob ok) via `--genomad_genes "path/to/*_virus_genes.tsv"` to populate the score: geNomad's `virus_hallmark` and marker `.VV`/`.Vv`/`.vV`/`.vv` suffixes map to VirSorter category `0`/`1`. The DRAM-v→geNomad join tries direct gene-id equality first, falling back to position-overlap on the parent assembly contig — CheckV-trimmed provirus contigs (`<contig>|provirus_<start>_<end>`) are handled correctly either way.

**Catalog vs per-sample mode.** If you ran DRAM on a multi-sample catalog whose contigs were renamed `<sample>_<orig>` upstream (e.g. by [tpall/nf-virome](https://github.com/tpall/nf-virome)'s `CONCAT_FASTAS`), pass `--genomad_filename_prefix true` so each `<sample>_virus_genes.tsv` filename is used to derive a sample prefix prepended to its gene ids before joining. Per-sample DRAM-v runs leave this off (default).

`--max_auxiliary_score` (default 3) sets the upper bound on scores kept in the `--amg_only` distillate; set to 5 to disable.
